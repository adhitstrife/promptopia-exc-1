    ...
    <nav className="flex-between w-full mb-16 pt-3">
        ...
        <div className="sm:flex hidden">
            { session?.user ? (
                <div className="flex gap-3 md:gap-5">
                    ...
                    <Link href="/profile">
                        <Image src={session?.user.image} width={37} height={30} className="rounded-full" alt="profile" />
                    </Link>
                </div>
            ) : (
                <>
        ...
        <div className="sm:hidden flex relative">
            {session?.user ? (
                <div className="">
                    <div>
                        <Image src={session?.user.image} width={37} height={30} className="rounded-full" alt="profile" onClick={() => settoggleDropdown((prev) => !prev)} />
                    </div>
